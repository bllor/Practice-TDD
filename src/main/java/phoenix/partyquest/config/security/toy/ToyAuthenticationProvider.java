...
public class ToyAuthenticationProvider implements AuthenticationProvider {
    ...
    private final ToyUserDetailService userDetailService;
    @Override
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        String uid = (String) authentication.getPrincipal();
        String password = (String) authentication.getCredentials();
        ToyUserDetails findMember = (ToyUserDetails) userDetailService.loadUserByUsername(uid);

        if (passwordEncoder.matches(password, findMember.getPassword())) {
            findMember.getToyMember().setPassword(""); // password는 SecurityContextHolder(세션)에 돌아다니지 않게 한다.
